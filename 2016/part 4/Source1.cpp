...
int main()
{
	...
	vector<pair<string,int>> decryptedStrings;
	...
	while (getline(file, line))
	{
		...
		{
			for (int i = 0; i < concacenatedString.size(); ++i)
			{
				int rotateCount = 0;

				do
				{
					if (concacenatedString[i] != 'z')
					{
						concacenatedString[i] += 1;
					}
					else
					{
						concacenatedString[i] = 'a';
					}
					++rotateCount;
				} while (rotateCount != sectorID);
			...
			decryptedStrings.push_back(make_pair(concacenatedString,sectorID));
