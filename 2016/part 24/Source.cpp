...
struct Current
{
	...
	bool operator==(const Current& other) const
	{
		...
		if (map == other.map && visitedNumbers == other.visitedNumbers &&
			currentCoord == other.currentCoord && steps == other.steps)
		{
			result = true;
		}
...
int main()
{
	...
	while (!queue.empty() && !found)
	{
		...
		/* get manhattan distance from current point to every number and find shortest */
		if (shortest == 'f')
		{
			shortest = ShortestDistance(current.currentCoord, coordList);
			cout << shortest << '\n';
		}

		if (current.visitedNumbers.find(shortest) != current.visitedNumbers.end())
		{

			for (int i = 0; i < coordList.size(); ++i)
			{
				if (coordList[i].first == shortest - '0')
				{
					coordList.erase(coordList.begin() + i);
				}
			}
			shortest = ShortestDistance(current.currentCoord, coordList);
			cout << shortest << '\n';

			std::queue<Current> empty;
